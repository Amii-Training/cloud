...
9. Now it's time to build and publish your application to Test Python Package Index. 
  ...
  You can see if your package has been successfully uploaded by navigating to the URL https://test.pypi.org/project/hospitalService. Note that the hospitalService is the name of the project.
...
Modify line 8 to download and install the application that we published to the PyPI index in previous step
...
Run `docker push <username>/hospital_service:1.0.0` to publish your hospital_service image to docker hub.
...
29. It's time to deploy your application. Execute `kubectl apply -f deployment.yaml` followed by `kubectl apply -f services.yaml`

30. To access the exposed service, we need to add a security rule to allow inbound traffic to your cluster. 
...
31. It should take you to Security Groups page. Here, click on the Security group ID
![alt text](images/sg_02.png)

32. This should show the details of the security group. From here, click on `Edit Inbound Rules` to add a new security rule
![alt text](images/sg_03.png)

33. Click on `Add rule` and define a new rule to allow All traffic as shown below (Type: All traffic, Source: Anywhere) and when you are done click on `Save rules`
